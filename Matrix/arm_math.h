...
inline void arm_mat_add_f32(const arm_matrix_instance_f32 *self, const arm_matrix_instance_f32 *other, arm_matrix_instance_f32 *result) {
//    auto self_data = (float32_t (*)[self->numCols])(self->pData);
    result->numCols = self->numCols;
    result->numRows = self->numRows;
    for (int i = 0; i < self->numRows; ++i) {
        for (int j = 0; j < self->numCols; ++j) {
            result->pData[i * self->numCols + j] = self->pData[i * self->numCols + j] + other->pData[i * self->numCols + j];
        }
    }
};

inline void arm_mat_sub_f32(const arm_matrix_instance_f32 *self, const arm_matrix_instance_f32 *other, arm_matrix_instance_f32 *result) {
    result->numCols = self->numCols;
    result->numRows = self->numRows;
    for (int i = 0; i < self->numRows; ++i) {
        for (int j = 0; j < self->numCols; ++j) {
...
inline void arm_mat_mult_f32(const arm_matrix_instance_f32 *self, const arm_matrix_instance_f32 *other, arm_matrix_instance_f32 *result) {
    result->numRows = self->numRows;
...
inline void arm_mat_scale_f32(const arm_matrix_instance_f32 *self, float scale, arm_matrix_instance_f32 *result) {
    result->numRows = self->numRows;
    result->numCols = self->numCols;
    for (int i = 0; i < self->numRows; ++i) {
        for (int j = 0; j < self->numCols; ++j) {
...
inline void arm_mat_trans_f32(const arm_matrix_instance_f32 *self, arm_matrix_instance_f32 *result) {
...
static arm_status arm_mat_inverse_f32(  const arm_matrix_instance_f32 * pSrc,
